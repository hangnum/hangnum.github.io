...
*  **原文链接:** [Cross-Modal Translation and Alignment for Survival Analysis](/assets/pdf/Cross-Modal%20Translation%20and%20Alignment%20for%20Survivalpdf)
...
*   **引导式融合 (Guided Fusion):** 通常以基因组学数据作为"查询"（Query），去指导模型关注病理图像中的特定区域。这种非对称的方法存在两大问题：
    ...
    2. **性能瓶颈:** 当引导模态（如基因组学）的预测性能不如被引导模态（病理学）时，用较差的信息去"污染"较好的信息，反而会限制模型的整体性能。
...
**2. 数据处理与特征提取:**
*   **病理图像 (Pathology):** 将WSI切成大量不重叠的512x512小块（patches），使用在ImageNet上预训练的ResNet-50提取每个patch的特征，得到一组patch特征 $P = \{p_1, p_2, \ldots, p_M\}$。
*   **基因组学 (Genomics):** 将RNA-seq, CNV, SNV等数据根据生物学功能（如肿瘤抑制、癌变等）进行分组，得到一组基因序列特征 $G = \{g_1, g_2, \ldots, g_K\}$。

**3. 模态内编码器 (Intra-Modal Encoders):**
*   **目的:** 分别聚合每种模态内部的信息，生成一个紧凑的全局表征。
*   **结构:** 采用基于自注意力机制的Transformer结构。输入为各模态的特征集合外加一个可学习的`[class]` token。
*   **输出:** 两个模态内表征向量：病理表征 $\mathbf{p}$ 和基因组表征 $\mathbf{g}$。
...
*   **机制:** 计算两个注意力图 $H_p$ 和 $H_g$。
    *   $H_p$ (Genomics-to-Pathology): 表示基因组学特征对病理patch特征的注意力权重。
    *   $H_g$ (Pathology-to-Genomics): 表示病理patch特征对基因组学特征的注意力权重。
*   **输出:**
    *   **$\tilde{P}$**: 从病理特征中提取的、与基因组学相关的特征集合。
    *   **$\tilde{G}$**: 从基因组学特征中提取的、与病理学相关的特征集合。

**5. 跨模态解码器 (Cross-Modal Decoders):**
*   **目的:** 将从注意力模块中提取的"相关信息"**翻译**成目标模态的表征形式。这个过程模拟了病理学家根据病理图像推断基因表达，或生物学家根据基因表达想象病理表型的过程。
*   **病理解码器:** 输入**$\tilde{P}$**（基因组学相关信息），输出一个跨模态表征 **$\hat{g}$**。这个 **$\hat{g}$** 是在病理信息的基础上"生成"的、与基因组表征 $\mathbf{g}$ 对齐的向量。
*   **基因组解码器:** 输入**$\tilde{G}$**（病理学相关信息），输出一个跨模态表征 **$\hat{p}$**。这个 **$\hat{p}$** 是在基因组信息的基础上"生成"的、与病理表征 $\mathbf{p}$ 对齐的向量。

**6. 特征对齐与融合 (Feature Alignment and Fusion):**
*   **对齐 (Alignment):** 为了保证"翻译"的质量，模型强制要求翻译生成的跨模态表征与原始的模态内表征尽可能相似。这通过一个L1损失函数 $\mathcal{L}_{\text{sim}} = \|\mathbf{p} - \hat{\mathbf{p}}\|_1 + \|\mathbf{g} - \hat{\mathbf{g}}\|_1$ 来实现。
*   **关键技术 (Tensor Detaching):** 在计算 $\mathcal{L}_{\text{sim}}$ 时，原始表征 $\mathbf{p}$ 和 $\mathbf{g}$ 会从计算图中**分离(detach)**。这意味着梯度只会从 $\mathcal{L}_{\text{sim}}$ 反向传播到解码器（用于优化"翻译"过程），而不会传播到编码器。这是一个非常精妙的设计，它实现了**单向优化**：$\hat{\mathbf{p}} \rightarrow \mathbf{p}$，$\hat{\mathbf{g}} \rightarrow \mathbf{g}$。这可以防止编码器为了最小化$\mathcal{L}_{\text{sim}}$而只学习模态间的共享信息，从而丢失各自独特的互补信息。
*   **融合 (Fusion):** 最终，将原始模态内表征与"翻译"生成的跨模态表征进行融合（相加后平均），以增强和校准原始特征。融合后的特征被送入一个MLP进行最终的生存预测。
    *   融合特征: $(\mathbf{p} + \hat{\mathbf{p}})/2$ 和 $(\mathbf{g} + \hat{\mathbf{g}})/2$

**7. 损失函数:**
总损失由两部分组成：生存预测损失（$\mathcal{L}_{\text{sur}}$，采用负对数似然损失）和对齐损失（$\mathcal{L}_{\text{sim}}$）。
$\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{sur}} + \alpha \cdot \mathcal{L}_{\text{sim}}$，其中 $\alpha$ 是一个超参数。
...
*   **消融实验 (Ablation Studies, Table 2):**
    ...
    *   **去除对齐约束 ($\mathcal{L}_{\text{sim}}$):** 性能下降，说明保证"翻译"质量的约束是必要的。
    *   **去除Tensor Detaching:** **性能出现严重下降**。这是最重要的消融实验，强有力地证明了单向对齐是防止模型坍缩、学习到互补信息的关键。若没有它，模型会倾向于让$\mathbf{p}$和$\hat{\mathbf{p}}$学习完全相同的东西，失去了多模态的意义。
