...
*  **原文链接:** [Cohort-Individual Cooperative Learning for Multimodal Cancer Survival Analysis](/assets/pdf/Cohort-Individual_Cooperative_Learning_for_Multimodal_Cancer_Survival_Analysis.pdf)
...
*   **基因组数据 (Genomics):**
    *   将RNA-seq, CNV, SNV等数据分成6个子序列（如肿瘤抑制、癌基因等），然后使用**自归一化神经网络 (SNN)** 进行特征提取。SNN是一种能够自我稳定的网络结构，适合处理这类高维表格数据。最后通过全连接层融合成一个单一的特征向量 $F_g$。

##### **3.3 多模态知识分解 (MKD)**

这是CCL的第一个核心创新。模型设计了四个独立的编码器来解耦 $F_p$ (病理特征) 和 $F_g$ (基因组特征)。

*   **独特性知识 (Uniqueness, P and G):**
    *   $P = \Phi_p(F_p)$ 和 $G = \Phi_g(F_g)$。这两个编码器（通常是简单的MLP）分别只接收单一模态的输入，旨在捕获该模态独有的、无法从另一模态推断的信息。

*   **冗余/公共知识 (Common, C) 和 协同知识 (Synergistic, S):**
    *   $C = \Phi_c(F_p, F_g)$ 和 $S = \Phi_s(F_p, F_g)$。这两个编码器都接收双模态输入。它们内部使用了一个**共注意力机制 (Co-attention)** (见Figure 4)。首先计算 $F_p$ 和 $F_g$ 之间的注意力矩阵 $A$，然后 $A$ 分别生成对 $F_p$ 和 $F_g$ 的注意力权重，加权求和得到融合特征。
    *   **关键区别**：虽然 $\Phi_c$ 和 $\Phi_s$ 结构相同，但它们的**参数是独立的**，并且将受到**完全不同的监督信号**（来自CGM模块），从而强迫它们分别学习"冗余"和"协同"两种不同的知识。
...
*   **知识层面指导 (Knowledge-Level Guidance):**
    *   通过一个**相似性约束损失 $l_k$ (式4)** 来定义四种知识的"身份"。你的笔记对此的理解非常到位：
...
**总损失函数**: $L = L_{\text{surv}} + \alpha \cdot L_{\text{cohort}}$，其中 $L_{\text{cohort}} = l_k + l_p$。
